...
- hk with input predictions on and off for warm and next-commit timings; `hk-pull` separately checks every prediction
- The hk, Lance, OpenDAL, Substrate and SurrealDB builds through mbx at its latest release; hk includes same-checkout and next-commit rebuilds
...
To run these six comparisons in CI, dispatch **Bench** with `hk-comparison`.
Individual scenarios can also be selected. They use the same
pinned hk commits and Rust toolchain. Each job exports the existing benchmark
reports and OTLP metrics. Jobs run on separate runners, so compare several runs
before attributing small timing differences to a cache setting.
