...
normal Kache/fallback pipeline. Learned units—and eligible user-facing test/bin
units already excluded by `cache_executables=false`—run directly through
Kache, so one implementation owns their incremental state and lock.
