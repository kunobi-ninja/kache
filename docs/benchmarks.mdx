...
The pull-request comment opens with one table per subject: this PR, the base
branch, the change between them, then every other tool, for isolated and
contended builds. The change column names a change only when the paired test
below confirms it, and shows its 95% interval; with fewer than five pairs it
reads "no verdict". Every PR gets six paired runs of this PR and its base,
which is enough for a verdict on the warm builds, while cold builds get two
and other tools and contention one. The bench label measures everything six
times. Contention counters, disk use and the paired test sit in folded
sections below; with more than one sample, each time shows its range. The
comment names this PR and the base branch; the data calls them `head` and
`base`.
...
reported as an improvement. Other timing results carry **no verdict**. The short suite applies these rules to both warm
...
samples carry no verdict. More duplicate Kache key compilations, or more
