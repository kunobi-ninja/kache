...
def renderer_from(staging):
    ...
    spec = importlib.util.spec_from_file_location(
        "perf_gate_report", staging / "bench" / "report.py"
    )
...
# What the gate copies out of `scripts/` to measure with: the package, the
# entry points that put it on the path, and the contention driver that still
# lives beside it. `perf-gate.yml` stages the same list.
INSTRUMENT_SCRIPTS = ("bench-short.py", "bench-contention.py", "perf-gate-report.py")
...
def copy_instrument_scripts(scripts, staging):
    """Stage the comparison scripts so their entry points run from `staging`.

    The entry points add their own directory to the path and import from the
    `bench` package beside them, so the package is copied whole, without its
    tests. An instrument commit from before the package existed has only the
    standalone scripts, and those still run on their own.
    """
    package = scripts / "bench"
    if package.is_dir():
        shutil.copytree(
            package,
            staging / "bench",
    ...
    for name in INSTRUMENT_SCRIPTS:
