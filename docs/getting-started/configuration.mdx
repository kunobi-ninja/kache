...
To reclaim the blocks, remove the retaining build output. `kache targets` lists every target directory Kache has seen, how long it has been idle, and what deleting it frees, and marks targets of deleted worktrees. For known stale targets, preview the deletion first:
...
| `KACHE_BUILD_SCRIPT_HERMETIC` | none | `false` | On Unix, run cached build scripts with `OUT_DIR` in a shared directory named by their inputs, so a run is shared across target directories. See [Build scripts](/how-it-works/build-scripts). |
