...
5. on a miss, joins a machine-wide flight, takes a memory-weighted permit and a per-key lock, and checks locally again
...
Identical output bytes share one blob. Restores first try copy-on-write cloning. Unix can use a restricted hardlink fallback for immutable artifacts, and Linux also for proc-macro libraries and build-script binaries. Everything else is copied. Other executables and loadable libraries are never hardlinked because later tools may mutate them.

See [Deduplication](/docs/deduplication) for platform details.

A miss stores its outputs without waiting for an fsync. The daemon flushes the new blobs and metadata to disk on a two-second sweep and marks those entries durable, and `kache gc` flushes whatever is still pending. Without a reachable daemon the wrapper flushes the entry itself, so a store used without a daemon does not accumulate unflushed entries. Until an entry is flushed, a hit on it verifies every blob against its content hash before restoring, so a crash in between costs a recompile, never a wrong artifact. `[cache] deferred_durability = false` flushes inside the compile instead.
