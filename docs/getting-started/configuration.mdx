...
On a host where several jobs share one cache, such as a CI machine with several runner slots, the same payload also describes the machine's shared cache: `kache.cache.index.size` (`index.db` plus `-wal`), `kache.cache.index.wal.size` (the `-wal` alone; one that stays large means checkpoints are not keeping up), `kache.cache.index.rowid_high_water` per table, and the last GC run as gauges (`kache.cache.gc.last_run.*`: when it ran, entries evicted, bytes freed, evictions that failed and those that lost the index write lock, duration, and time spent in eviction writes). The payload has no host attribute; whatever ships the file adds host identity. These figures never wait: the index is read `query_only` with a 25 ms busy timeout, and a figure it cannot answer at once is left out. `kache stats` shows the same figures, and `kache stats --json` carries them as `index_bytes`, `index_wal_bytes`, `index_rowid_high_water` and `gc`.

Read `rowid_high_water` as a write counter, not a row count. It is the largest rowid in each table. Several tables (`file_hashes`, `cc_preprocess_memos`, `input_predictions`, `eviction_tombstones`) are written with `INSERT OR REPLACE`, which gives a replaced row a new rowid, so the figure grows with every insert and every replacement and does not fall when rows are deleted. For how big the index is, use `kache.cache.index.size`.
...
Every GC driver (the daemon's sweep and its eviction after an upload, the auto-GC worker, and `kache gc` run locally, `--stale-schema` and `--max-age` included) records its run in `gc_stats.json` in the cache dir, including evictions that failed and how many of those lost the index write lock to running builds. With `record_sessions` on, each run also appends one line to `telemetry/gc-runs.jsonl` in the cache dir (time, driver, entries evicted, bytes freed, failed and lock-contended evictions, pinned and unreclaimable entries, duration, and time spent in eviction writes), rotated like the other logs. `gc_stats.json` keeps only the latest run; the history is what a backend adds up.

None of this runs unless you ask for it. The machine figures are computed only by `kache telemetry write` and `kache stats`, and both only read: file sizes, `gc_stats.json`, and one seek per table for its row mark, never a count over a table. Nothing is sent over the network, and the daemon does nothing new. `telemetry/sessions.jsonl` exists only after a `kache report --record` or with `record_sessions` on, and `telemetry/gc-runs.jsonl` only with `record_sessions` on. The one change that is always on is GC accounting: the auto-GC worker and `kache gc` now update the same `gc_stats.json` the daemon already wrote, once per GC run.
