...
- `[cache.remote]` and `[cache.planner]` are replaced whole. If the chosen file declares one, none of the host file's keys for it apply.
- A remote set only through `KACHE_S3_*` variables also replaces the host remote, as long as the chosen file declares no `[cache.remote]`.
