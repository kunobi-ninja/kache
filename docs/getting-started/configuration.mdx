...
- **Exact names, or a trailing `*` prefix glob.** `BOLTFFI_*` selects every var starting with `BOLTFFI_`; `APP_MODE` selects only that name. A `*` anywhere else is a literal character and kache warns about it. Matching is case-insensitive, so a Windows environment behaves like a Unix one.
...
- **Only vars actually set are folded**, as sorted `NAME=VALUE` pairs. Declaration order in the file doesn't matter.
- **Values are path-normalized** first, so a declared var holding a checkout path (`BOLTFFI_ROOT=/home/alice/proj`) doesn't pin the entry to one machine.
...
Keep the list to vars that genuinely change what a macro emits. Naming something that varies per build — a timestamp, a job id, `PWD` — gives every compile its own key and disables caching for the whole workspace.
