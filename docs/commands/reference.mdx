...
Runs garbage collection. Without `--max-age`, applies the automatic policy: size-pressure eviction plus age retention for entries idle beyond `KACHE_GC_MAX_AGE_HOURS` (default 7 days; `0` disables age retention). Size pressure fires above `KACHE_MAX_SIZE` and evicts down to 90% of that cap using physical bytes. Duplicate cleanup is bounded by the same target and removes an entry only when backfilled metadata proves positive physical reclaim; unknown legacy entries are kept. With `--max-age`, removes all entries older than the specified duration regardless of store size.
