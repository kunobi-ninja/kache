...
Runs garbage collection. Without `--max-age`, this is the automatic policy: size-pressure eviction, which fires once the store's physical size on disk exceeds `KACHE_MAX_SIZE` (100%) and evicts down to 90% of it — a real hysteresis band, not the same line for both trigger and stop condition — composed with age-retention eviction of anything idle past `KACHE_GC_MAX_AGE_HOURS` (default 7 days, `0` disables it). Both are measured in physical bytes (deduplicated blobs counted once), and size-pressure ranking prefers entries whose removal actually frees disk space — an entry whose blobs are all shared with other entries frees nothing and is kept; the same protection applies to duplicate-entry cleanup, which only runs once physical size exceeds the cap and only removes a duplicate whose blob isn't still held by another entry. With `--max-age`, removes all entries older than the specified duration regardless of store size, and runs only that policy.
