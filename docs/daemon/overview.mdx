...
Pick a short directory that only you can write to — `$XDG_RUNTIME_DIR/kache/daemon.sock` on Linux, `~/Library/Caches/kache/daemon.sock` on macOS. Avoid `/tmp` and other shared directories: the daemon keeps its lock, state, and log files next to the socket, and in a shared directory another local user can create those files, or bind the socket first so kache reads it as "already running". kache warns at daemon start if the directory is world-writable or owned by someone else, and if the resulting path is too long to bind. The value must be absolute — a relative path would resolve against each caller's working directory, and the wrapper (running in whichever crate cargo is building) and the daemon do not share one. Unusable values are logged and fall back to `<cache_dir>/daemon.sock`.
