...
The daemon watches the config file and restarts itself when it changes. This retains exact remote hits and uploads without speculative warming. If you use `KACHE_PREFETCH_ENABLED=0` instead, run `kache daemon restart` after changing the environment: an already-running daemon cannot see new environment values, and client-side `kache stats` may otherwise report the new policy while the old daemon keeps warming.
