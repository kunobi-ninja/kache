...
On a host where several jobs share one cache, such as a CI machine with several runner slots, the same payload also describes the machine's shared cache: `kache.cache.index.size` (`index.db` plus `-wal`), `kache.cache.index.rowid_high_water` per table, the last GC run as `kache.cache.gc.last_run.*`, and GC totals as cumulative sums (`kache.cache.gc.runs`, `.entries_evicted`, `.bytes_freed`, `.entries_failed`, `.entries_locked`). The payload has no host attribute; whatever ships the file adds host identity. These figures never wait: the index is read `query_only` with a 25 ms busy timeout, and a figure it cannot answer at once is left out. `kache stats` shows the same figures, and `kache stats --json` carries them as `index_bytes`, `index_rowid_high_water` and `gc`.
...
After rendering the report as usual, it appends one JSON line to `telemetry/sessions.jsonl` in the cache dir: the report's summary and timing breakdown (hit overhead, key, dep-info pre-pass, restore, wait, prediction mismatches), plus load average, CPUs and index size at that moment. Use it at the end of a CI job: the report reads the runtime dir, which the job's cleanup deletes. The root is recorded as a hash, never as a path. Without `--record`, `kache report` writes nothing.

Every GC driver (the daemon's sweep and its eviction after an upload, the auto-GC worker, and `kache gc` run locally, `--stale-schema` and `--max-age` included) records its run in `gc_stats.json` in the cache dir, including evictions that failed and how many of those lost the index write lock to running builds.

None of this runs unless you ask for it. The machine figures are computed only by `kache telemetry write` and `kache stats`, and both only read: file sizes, `gc_stats.json`, and one seek per table for its row mark, never a count over a table. Nothing is sent over the network, and the daemon does nothing new. `telemetry/sessions.jsonl` exists only after a `kache report --record`. The one change that is always on is GC accounting: the auto-GC worker and `kache gc` now update the same `gc_stats.json` the daemon already wrote, once per GC run.
