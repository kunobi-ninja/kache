...
| `KACHE_INDEX_AUTO_COMPACT` | `cache.index_auto_compact` | `true` | Let the daemon compact `index.db` when free pages dominate it. See [Index compaction](#index-compaction). |
...
- On a quiet machine: no compile holds a scheduler permit, the daemon has received no build request for 60 seconds, and the live part of the index is at most 8 GiB. Stats polling and an open `kache monitor` do not count as build requests. If anything else is using the index at that moment, the attempt stops at once and a later check retries.
...
The daemon checks 60 seconds after it starts and every 5 minutes after that. Compaction never overlaps a GC sweep. It needs free disk of about twice the live index size, not the file size: a 45 GB index with 1 GB live needs about 2 GB. Without that room the attempt is skipped and retried later. The free pages stay reusable in the meantime, so a skipped compaction costs disk space only.
...
Set `cache.index_auto_compact = false` or `KACHE_INDEX_AUTO_COMPACT=0` to turn this off. `kache doctor --repair` compacts on demand either way.
