...
During a rolling upgrade, a bare command rejects an older daemon response that lacks the per-policy result and reruns the effective policy locally under the same cross-process GC lock.
