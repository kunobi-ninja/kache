...
A machine owner can set keys for every build on the machine in `/etc/kache/config.toml`. Kache merges it with the chosen file key by key: a key the chosen file sets wins, and a key it leaves unset comes from the host file. Environment variables still win over both.

The host file reaches CI jobs too. kache-action points `KACHE_CONFIG` at a file of its own that only sets `[cache.remote]`, so a host file like this applies to every job on the runner:
...
| none | `cache.volumes` | unset | Volume-local store shards, e.g. `[cache.volumes] "D:" = "D:/kache-store"`. Builds on a mapped volume share inodes there instead of copying across mounts. Unmapped volumes use the main store. GC sweeps each shard with its own budget; see [Volume shards](#volume-shards). |
...
| `KACHE_AUTO_CLEAN_ORPHANED_TARGETS` | `cache.auto_clean_orphaned_targets` | `true` | Let the daemon remove target directories whose workspace was deleted. See [Target directory cleanup](#target-directory-cleanup). |
| `KACHE_AUTO_CLEAN_IDLE_TARGETS_DAYS` | `cache.auto_clean_idle_targets_days` | `0` | Let the daemon also remove target directories no build has used for this many days; `0` disables it |
...
Several `cargo test` runs on one machine each start a test thread per CPU. `kache test-runner` makes test binaries wait for slots in the scheduler's pool instead, the same pool compiles use.

On Unix, `kache init` offers to turn it on for every project. It adds this line to your shell startup file, with your machine's target in the name:

```bash
[ -n "${CARGO_TARGET_AARCH64_APPLE_DARWIN_RUNNER-}" ] || CARGO_TARGET_AARCH64_APPLE_DARWIN_RUNNER='kache test-runner'; export CARGO_TARGET_AARCH64_APPLE_DARWIN_RUNNER
```

It reaches every process started from a shell that reads that file, including coding agents launched from a terminal. A value you set earlier in the file is kept.

Cargo ranks the variable above a project's own `[target.<triple>]` or `[target.'cfg(..)']` runner, so the test runner looks that runner up and runs the test binary under it. A project that tests under `sudo -E` or an emulator keeps doing so. Cargo reads configuration from the directory it was started in, which a test binary is not told, so the lookup reads `.cargo/config.toml` upward from both the package directory and `$PWD`, plus the files they `include` and `$CARGO_HOME/config.toml`. `cfg` runners are matched against `rustc --print cfg` with the `--cfg` flags from `RUSTFLAGS`.

When the test runner cannot tell which runner Cargo would have used, the test fails with `cannot tell which runner Cargo would use` and the reason. That happens when:

- the package directory and `$PWD` configure different runners, as with `cargo test --manifest-path` run from outside the project;
- more than one `cfg` runner matches;
- rustc cannot report the target's cfg, or a `cfg` runner's expression cannot be read;
- `--cfg` flags come from `build.rustflags`, `target.<triple>.rustflags` or `CARGO_BUILD_RUSTFLAGS`, which the test runner does not evaluate.

Unset the variable in that shell to let Cargo choose.

To turn it on for one project instead, set it as Cargo's target runner in the project's `.cargo/config.toml`:
...
Cargo rejects two `cfg` runners that both match a target, so if the project already sets one, replace it rather than adding a second. Do not put a `cfg(all())` runner in `$CARGO_HOME/config.toml`: every project with its own `cfg` runner, embedded projects using probe-rs among them, would then fail to load its configuration.
...
- Builds and test binaries started inside a test count against that test's slots and do not wait for slots of their own. A nested `cargo build` can therefore run more compiles at once than its test holds slots.
- While tests hold slots, a compile that has waited asks for no more slots than the tests leave free, and never fewer than the reserve. A test that waits on a build therefore cannot block that build.
- Exit codes, signals, stdin and Ctrl-C behave as if the binary ran directly. The runner writes nothing to stdout.

The runner starts these directly, without waiting for slots: binaries Cargo does not name with a hash (`cargo run`, doctests), test listing (`--list`, which nextest uses), benchmarks (`--bench`), and every run when `KACHE_SCHEDULER=0` or `[cache] scheduler = false`.
...
By default, GC keeps an entry when removing its last store name would reclaim zero bytes. This preserves a usable cache hit. It also means the registered store size can remain above `local_max_size` until the target output is removed. `kache gc` reports these entries as unreclaimable, with the bytes they hold, and separates store bytes removed from disk bytes reclaimed.

A size-driven sweep leaves those held bytes out of its budget. Before it removes anything, it finds the entries whose last blob a build output still holds, and it evicts until the rest of the store is back under the target. A store whose registered size is mostly held by target directories is therefore not evicted further: removing other entries would lose cache hits without bringing the held bytes down.

Automatic eviction has one trigger: a sweep starts when the store is above 110% of `local_max_size` and evicts down to 90%. Three things can ask for one: a build that has just stored an entry (checked at most every 5 minutes), the daemon after an upload, and the daemon's 6-hour sweep. `kache gc` evicts from 100%.

While a daemon is running, it does the sweeping. A build that finds the store over the trigger sends the daemon a hint and continues; it does not wait for the sweep, and it never starts a daemon for this. Hints that arrive during a sweep are dropped. With no daemon, or a daemon too old to accept the hint, the build starts a detached `kache gc`. That worker sweeps a second time two minutes later only if the first sweep had to skip entries a running build was using.

Every automatic sweep records the store size it ended at, whichever of the three started it. When an automatic sweep leaves the store over budget, the next automatic sweep waits 10 minutes. Each further sweep that ends over budget doubles the wait, up to 2 hours. The wait ends early when the store grows by more than 10% of `local_max_size`, and resets once a sweep brings the store back under budget. The wait applies to all three, so one sweep that cannot free enough holds the others back too. The 6-hour sweep still expires entries by `cache.gc_max_age_hours` and removes duplicates during a wait; only its size eviction is skipped. The state lives in `auto-gc-backoff.json` in the cache dir. `kache gc` ignores it.

`auto-gc-backoff.json` also keeps the bytes the last sweep found held by target directories. For 6 hours the trigger compares the store size minus those bytes with 110% of `local_max_size`, so a store over budget only on held bytes starts no sweep. Deleting a worktree frees what it held without changing the store size; after 6 hours the trigger uses the full size again, and the next sweep measures anew.
...
A sweep pauses for 150 ms after every 50 ms of index writes, and after each removal that finds a build holding the index write lock, so the sweep does not hold up builds that are storing entries.

To reclaim the blocks, remove the retaining build output. `kache targets` lists every target directory Kache has seen, how long it has been idle, and what deleting it frees. It also marks targets of deleted worktrees. For known stale targets, preview the deletion first:
...
Targets of deleted worktrees are offered at any age. `kache clean --orphans --yes` removes only those, which suits a hook that runs after `git worktree remove`.

If enforcing the store namespace limit matters more than retaining hits, let GC evict shared entries:
...
Every GC driver sweeps each `[cache.volumes]` shard as well as the main store: `kache gc`, the daemon's 6-hour sweep, its sweep after an upload or a build's hint, and the detached auto-GC worker. Each shard is swept like the main store, with the same age, duplicate, and size passes.

Each shard has its own budget. An explicit `local_max_size` or `KACHE_MAX_SIZE` applies to each shard as it is. Without one, a shard's budget is 5% of the filesystem that holds it, floored at 5GiB and capped at 100GiB, the same rule as the main store.

A shard keeps its own `gc.lock`, `auto-gc-backoff.json` and `gc_stats.json` in its store dir, so its trigger, backoff and held bytes do not mix with the main store's. `kache gc` prints one line per shard after the main store's.

GC skips a shard whose store dir has no `index.db`: a missing directory, an unmounted volume, or a shard no build has written to yet. It never creates a shard directory. A shard that fails to open or sweep is logged, and GC carries on with the others and the main store.

`kache stats`, `kache report`, `kache doctor` and `kache gc --json` still describe only the main store, and `kache gc --stale-schema` still covers only the main store.

### Daemon index maintenance

The daemon looks after `index.db` on its own. It checks 60 seconds after it starts, every 5 minutes after that, and once at shutdown. Each check runs two steps in order: it heals the blob index, then compacts the file. Both steps hold the index write lock while they work, so both wait for a quiet machine. A machine is quiet when no compile or test binary run through the [test runner](#test-runner) holds a scheduler slot, and the daemon has received no build request for 60 seconds. Stats polling and an open `kache monitor` do not count as build requests. Neither step overlaps a GC sweep.
...
A `VACUUM` blocks other index users while it runs, so the daemon runs one only in these cases:
...
Builds through Kache record the target directories they use; `kache targets` lists them. The daemon checks that list at most once an hour, when the machine is quiet, and removes:

- target directories whose workspace was deleted and that no build has used for a day, such as the target of a removed worktree built with its own `CARGO_TARGET_DIR`. The day keeps a worktree that is moved or recreated from losing its target. Set `cache.auto_clean_orphaned_targets = false` or `KACHE_AUTO_CLEAN_ORPHANED_TARGETS=0` to keep them.
- with `cache.auto_clean_idle_targets_days` set, target directories no build has used for that many days, whether or not their workspace still exists.

It applies the same checks as `kache clean --tracked --yes`. A directory is removed only while it is still the Cargo target directory that was recorded, never when it holds a `Cargo.toml` or `.git`, and never while a running build holds it. The daemon renames it aside and checks once more before deleting it, so a build that starts at that moment gets a new directory. A workspace counts as deleted only when looking it up finds nothing; a permission or network error does not count. The daemon log names each directory it removes and why.

The cleanup follows the configuration of the daemon that serves the cache directory.
...
| `KACHE_VERIFY_INPUT_PREDICTIONS` | none | off | `sampled` or `always`: also run the dep-info pre-pass for a validated prediction and count disagreements in the report. The pre-pass result is the one used |
| `KACHE_BUILD_SCRIPT_HERMETIC` | none | `false` | On Unix, run cached build scripts with `OUT_DIR` in a shared directory named by their inputs, so a run is shared across target directories. See [Build scripts](/docs/how-it-works/build-scripts). |
| `KACHE_BUILD_SCRIPT_CACHE` | none | `true` on Linux and macOS | Cache build-script runs: restore `OUT_DIR` and the script's output instead of running it when its binary, Cargo environment and declared `rerun-if-*` inputs match a recorded run. See [Build scripts](/docs/how-it-works/build-scripts). |
...
Use `KACHE_VERIFY` to check a cache-key change. Set it to `1` or `true` to recompile every rustc cache hit into a staging directory and diff those artifacts against the files just restored. Leave it unset (or `0` / `false`) in ordinary builds: each hit pays a full compile. There is no `kache build` command for this.
...
`KACHE_VERIFY_RESTORES` only re-hashes stored blobs before restore. It does not recompile.
...
`KACHE_OUT_DIR_ALIAS=0`. The hint comes from rustc's error output, so a compile
that Kache passes straight to rustc fails with rustc's error alone. That covers
a `bypass_*` or `exclude` match and a compile on the `preserve_incremental`
lane.
...
Values are hashed exactly and are not written to event logs. A value that changes on every job, such as a job ID, gives every job its own keys.
...
| Environment | TOML key | Default | Purpose |
| --- | --- | --- | --- |
| none | `cache.remote.type` | inferred: `filesystem` when `path` is set, otherwise `s3` | `s3` or `filesystem` |
...
| `KACHE_REMOTE_KEY_LISTING` | `cache.remote_key_listing` | `false` for S3, `true` for a filesystem remote | List the remote's keys, for the fallback planner's key index and packed-prefetch catalogs; on S3 this needs `s3:ListBucket` |
| `KACHE_REMOTE_KEY_CACHE_REFRESH_SECS` | `cache.remote_key_cache_refresh_secs` | `60` | Refresh the remote key index while listing is on; `0` means initial load only |
...
3. Your Kunobi session. Run `kache login` once: it trusts this planner and stores a session that the daemon refreshes from then on. The session is kept per planner client, so it never replaces another Kunobi tool's login.

Kache sends the OIDC token and the session only to a planner reached over HTTPS (or on loopback), and the session only to a planner you logged in to. A planner URL picked up from a project's config therefore cannot collect your credentials. Use `kache login --device` on a machine without a browser, and `kache logout` to revoke it.

A build never prompts for a login. Without a usable bearer the planner answers 401 and Kache plans locally, the same as when the planner is unreachable. If the planner's login issuer changes after you first trusted it, `kache login` refuses; run `kache login --retrust` when the change is expected.

## Telemetry

Kache does not push metrics to a collector. It writes OTLP JSON files that Kartero imports from CI artifacts, the same way it imports nightly bench results.

The two metric families stay separate:
...
After rendering the report as usual, it appends one JSON line to `telemetry/sessions.jsonl` in the cache dir. The line holds the report's summary and timing breakdown (hit overhead, key, dep-info pre-pass, restore, wait, prediction mismatches), plus load average, CPUs and index size at that moment. Use it at the end of a CI job: the report reads the runtime dir, which the job's cleanup deletes. The root is recorded as a hash, never as a path.

To record every report on a host without passing the flag, set `KACHE_RECORD_SESSIONS=1` or `[cache] record_sessions = true`. Recording is off by default: without the setting or `--record`, `kache report` writes nothing.
...
`kache telemetry write` reads index rows and files locally; it sends nothing over the network.
