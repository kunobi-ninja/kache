...
Globs are relative to the crate directory and may not escape it (`..` or absolute paths are rejected). A bare directory is fine — `.sqlx` and `.sqlx/` both mean "everything under it." The matched files' contents are folded into that crate's key.

This is **opt-in and scoped per crate**: a crate with no `kache.toml` is unaffected, and one crate's `extra_inputs` never touch a sibling crate's key. It is also **union-only** — a misdeclared glob can only cause an extra cache *miss* (a rebuild), never restore a wrong artifact. Files are folded by content hash, so moving the worktree doesn't bust the cache, and the declared patterns are folded too, so editing `kache.toml` itself re-keys.
