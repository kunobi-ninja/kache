...
It describes both projects as they are today and does not rank their speed. Test the same repository, toolchain, runner, and remote before deciding.
...
| C/C++ setup | `kache init` (PATH compiler-name shims on Unix) or `CC="kache cc"` | Prefix wrapper (`sccache gcc`); no ccache-style PATH farm |
...
| Fallback | Passes invocations it does not cache to another wrapper, such as sccache | No Kache-specific handoff needed |
...
If you need a compiler Kache does not cache yet, [use sccache as a fallback](#use-sccache-as-a-fallback). Kache caches what it supports and passes the rest to sccache, so one build can use both.
...
optional check respects `SCCACHE_SERVER_PORT`. With a Unix socket, an unidentified
server, or no sandbox query available, Kache runs the fallback as usual and retries
with the direct compiler if it fails. Kache does not start, stop, or change the
server during this check. A server that starts after the check can still inherit
its launcher's sandbox, so the retry is still needed. Use `KACHE_FALLBACK=off` to disable the fallback for a build.
