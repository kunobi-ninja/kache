...
| `KACHE_SOCKET_PATH` | — | `<cache_dir>/daemon.sock` | Daemon socket path, for when a deep `cache_dir` pushes the socket past the platform's unix-socket path limit. Operational and security-sensitive: it decides where clients look for the daemon, so `ignore_env` does **not** cover it (see [Daemon socket placement](/daemon/overview#communication)). Must be absolute; an unusable value warns and falls back to the default |
