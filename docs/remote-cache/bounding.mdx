...
- Expire by age since the object was written. kache never rewrites an entry's manifest or pack, so a hot entry expires too and is uploaded again on the next miss. Build manifests and prediction rows are written again each time they are used, so they expire only when idle. An expired prediction row costs one dep-info pre-pass. That is the same trade every object store's lifecycle makes; pick an age longer than the interval at which your builds change.
