...
pub(crate) async fn submit_timeline(
    ...
    if state.token.is_some() && identity.is_none() {
        ...
        return Err(refuse(
            ...
            "build timelines need the service's bearer token",
        ));
    }
...
mod tests {
    ...
    type StoredCall = (String, BuildTimeline, Vec<u8>);
    ...
    impl TimelineStore for RecordingRepository {
        async fn store_timeline(
            ...
            self.calls.lock().unwrap().push((server_id, record, body));
    ...
    async fn stores_a_submission_under_this_servers_name() {
        ...
        assert_eq!(calls[0].1.client_record_id, "r1");
