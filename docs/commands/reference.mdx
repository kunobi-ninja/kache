...
The payload also carries this host's shared cache: index size, its WAL size, and a rowid high-water mark per table (`kache.cache.index.*`; it grows with every insert and replacement, so it is not a row count) and the last GC run (`kache.cache.gc.last_run.*`, including evictions that failed, those that lost the index write lock, and the time its eviction writes took). Those figures never wait on a busy index.
