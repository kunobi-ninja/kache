...
For **gcc/clang on non-Windows hosts** the key is portable across machines and worktrees (paths are normalized via `-ffile-prefix-map`). On **Windows** — for gcc/clang and **clang-cl** alike — the key is currently **machine-local**: kache keeps literal paths in the key and injects no `-ffile-prefix-map`. clang-cl ignores the flag; and for gcc/clang the flag would rewrite `__FILE__` and mangle its `\` separators to `/`, silently changing the program-observable `__FILE__` ([#299](https://github.com/kunobi-ninja/kache/issues/299)). Correct local hits, but no cross-machine cc sharing on Windows yet.
