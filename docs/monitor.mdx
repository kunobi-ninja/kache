...
Cascade causes need per-dependency digests on the events, which `[cache] explain_miss = true` records. Without it the Why tab says so. A crate with no earlier compile in its own tree is compared with its latest build in another checkout of the project, so the first build in a new worktree can still name the crate that changed. Every figure on the tab describes the loaded history: with `--since`, a crate whose last compile is before the cutoff reads as having no earlier compile.
