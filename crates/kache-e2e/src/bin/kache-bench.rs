...
struct Args {
    ...
    /// Scratch directory for the clones, objdirs and cache (~55 GB).
    /// Lives under the repo's `tmp/` convention; gitignored. The whole
    /// directory tree (clone-clean, clone-a, clone-b, cache, snapshots)
    /// can be `rm -rf`ed at any time; subsequent runs re-derive what
    /// they need. `clone-clean` is the once-per-tag cached shallow
    /// reference; `clone-a` and `clone-b` are git worktrees off it.
...
fn main() -> Result<()> {
    ...
    let clone_clean = work_dir.join("clone-clean");
    ...
    eprintln!(
        "work dir    : {}  (~55 GB: clone-clean + 2 worktrees + objdirs + cache)",
        work_dir.display()
    );
    ...
    if args.skip_clone || args.retry {
        ...
        clone_firefox(&args.repo, &args.tag, &clone_clean, &clone_a, &clone_b)?;
    }
...
/// locally-cached `clone_clean` shallow clone.
///
/// The first run for a given `tag` pays the network cost once — a
/// `--depth=1 --branch=<tag>` clone into `clone_clean`. Every subsequent
...
///
/// Worktrees share `clone_clean`'s object database but each lives at
/// its own absolute path with independent working-tree files — the
...
fn clone_firefox(
    ...
    // 1. Bring clone-clean up to the requested tag (network cost,
    //    paid once per tag bump).
    if clone_clean_at_tag(clone_clean, tag)? {
        eprintln!("\n[bench] reusing clone-clean at {tag} (no network)");
    } else {
        if clone_clean.exists() {
            eprintln!(
                "\n[bench] clone-clean is missing or at the wrong tag — wiping and re-cloning"
            );
            std::fs::remove_dir_all(clone_clean)
                .with_context(|| format!("removing stale {}", clone_clean.display()))?;
        } else {
            eprintln!("\n[bench] no clone-clean yet — fetching Firefox {tag} (one-time cost)");
        }
        run(Command::new("git")
    ...
    for d in [clone_a, clone_b] {
        reset_worktree_path(clone_clean, d)?;
    }
...
/// True when `clone_clean` is a valid git repository whose HEAD is the
...
fn clone_clean_at_tag(clone_clean: &Path, tag: &str) -> Result<bool> {
    if !clone_clean.join(".git").exists() {
        return Ok(false);
    }
...
fn reset_worktree_path(clone_clean: &Path, target: &Path) -> Result<()> {
    if !target.exists() {
        return Ok(());
    }
    let _ = Command::new("git")
