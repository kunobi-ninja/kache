...
The Firefox sccache arm runs weekly rather than nightly, and whenever the
pinned sccache version changes. sccache is pinned in `mise.toml`, so that arm
measures the same binary every night while only Kache moves, and Kache against
Kache is what the per-pull-request perf gate reports. mbx tracks its latest
release, so it stays nightly.
