...
Without `--since`, the monitor opens with the last hour of builds. The monitor attempts to start the daemon for transfer statistics. It still reads the local store and event log if the daemon is unavailable.

<video src="https://raw.githubusercontent.com/kunobi-ninja/kache/main/assets/monitor.webm" aria-label="kache monitor following a build, then the Why, Projects, and Store tabs" autoPlay loop muted playsInline controls />
...
2. `Why` explains the selected build. Misses are collapsed to their causes, most frequent first: a failed store or a rejected lookup (a caching failure to fix), a dependency cascade named by the crate that actually changed, the crate's own changed inputs, no earlier compile in the loaded history, or unexplained. Passthroughs are grouped by reason, with queries such as `--print` kept apart from compiles. Crates that keep missing across builds of the same tree are listed as chronic, and every passthrough in the build is listed at the end. PgUp and PgDn scroll the whole tab.
...
A build is every event sharing one session id. The wrapper records one per workspace, and it covers the crates, build scripts, and C/C++ compiles of that workspace until it sits idle for five minutes, so Cargo commands run back to back count as one build. Events from older wrappers have no id and are grouped by build root with the same five-minute rule. Those builds carry a `~` after their name.

Cascade causes need per-dependency digests on the events, which `[cache] explain_miss = true` records. Without it the Why tab says so. When a crate has no earlier compile in its own tree, the Why tab compares it with its latest build in another build tree, such as another worktree of the project. A cascade is then named by the crate that changed, and that crate shows the input groups that differ. A crate whose inputs match the other tree still reads as having no earlier compile. Every figure on the tab describes the loaded history: with `--since`, a crate whose last compile is before the cutoff reads as having no earlier compile.
...
The monitor starts by following the top build, and the Builds panel title says "following the top build". Pressing Up past the first row returns to that mode. The top build is the newest running one, or the newest finished one when nothing is running.

Clicking a tab title selects it and the mouse wheel scrolls the current panel.

The `Lookups` strip under the Build events has one column per slice of the last five minutes (or of the `--since` window), with hits above misses on a shared scale. A quiet minute stays flat and a burst stands out.
