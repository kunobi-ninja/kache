...
contended builds. With one run of each, a change under 30% reads "within
noise", since identical builds vary that much on CI runners; a larger one is
shown as it is, and one the paired test confirms is bold. Contention counters,
disk use and the paired test sit in folded sections below; with more than one
sample, each time shows its range. The comment names this PR and the base
branch; the data calls them `head` and `base`.
...
reported as an improvement. Other timing results are **unconfirmed**, including
...
samples remain unconfirmed. More duplicate Kache key compilations, or more
