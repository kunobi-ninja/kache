...
`init` configures Cargo's Rust wrapper. On Unix it also sets `HOST_CC` and `HOST_CXX` when unset, so build-script C and C++ is cached, and offers compiler-name shims with a `PATH` line for your shell's startup file. It also offers a login service. It does not set `CC` or `CXX`. See [C and C++](/docs/getting-started/c-cpp) for native build setup.
