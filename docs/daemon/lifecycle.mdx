...
- **Upload jobs**: the wrapper records each validated intent under `<cache_dir>/upload-queue` before sending it. The daemon retires that file only after the object is confirmed present, replays remaining intents after restart, and defers availability failures without failing the build.
