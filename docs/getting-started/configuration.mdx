...
| `KACHE_REMOTE_RESTORE_TIMEOUT_SECS` | `cache.remote_restore_timeout_secs` | `300` | One monotonic deadline for a remote operation, including concurrency queues, HEAD/LIST, GET/PUT, and restore decompression/extraction. The wrapper sends its budget to the daemon, which uses the stricter client/daemon value. Demand expiry reports a miss so rustc recompiles; background work is deferred or skipped safely. `0` disables this operation deadline (transport connect/read-inactivity limits still apply) |
