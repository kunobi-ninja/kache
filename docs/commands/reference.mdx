...
remote, prefetch, and remote-index settings. If this process resolved a
different config path or value, stderr names both sides and the daemon's value
remains authoritative. If no daemon is reachable, the command announces when
it starts one and notes that a manually started daemon inherits that process's
environment.
...
Runs garbage collection. Without `--max-age`, evicts stale, rarely-used entries until the store's physical size on disk is under 90% of `KACHE_MAX_SIZE`. Both the trigger and the stop condition are measured in physical bytes (deduplicated blobs counted once), and the ranking prefers entries whose removal actually frees disk space — an entry whose blobs are all shared with other entries frees nothing and is kept. With `--max-age`, removes all entries older than the specified duration regardless of store size.
