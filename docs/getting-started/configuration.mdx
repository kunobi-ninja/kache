...
| `KACHE_MIN_STORE_COMPILE_MS` | `cache.min_store_compile_ms` | `0` | Put-side admission control: a compile faster than this many milliseconds is not stored. `0` (default) stores everything. This is separate from `KACHE_MIN_COMPILE_MS`, which gates remote prefetch downloads |
