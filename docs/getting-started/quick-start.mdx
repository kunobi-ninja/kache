...
<video src="https://raw.githubusercontent.com/kunobi-ninja/kache/main/assets/demo.webm" aria-label="a second worktree of the same commit building from cache hits, then the build report" autoPlay loop muted playsInline controls />
...
`kache init` configures Cargo's Rust wrapper. On Unix it also sets `HOST_CC` and `HOST_CXX` when unset, so Kache caches the C and C++ that build scripts compile, and offers compiler-name shims with a `PATH` line for your shell's startup file and [test pacing](/docs/getting-started/configuration#test-runner). It also offers a login service. It does not set `CC` or `CXX`. See [C and C++](/docs/getting-started/c-cpp) for native build setup.
...
The explicit target paths keep the two builds separate even if you normally use a shared target directory. Cargo must request outputs for the second tree, so that Kache can restore them. Running Cargo again with an already populated target may do no compiler work and produce no new Kache events.
...
The report covers the second tree's recorded events from the last 24 hours. To see only the latest build session:

```bash
kache report --last-build --root "$kache_trial/second"
```

A session can include nearby or concurrent Cargo commands in one root; it is not an exact Cargo invocation ID. Only completed, retained compiler events appear. See the [command reference](/docs/commands/reference#kache-report) for the selection rules.
...
If the second build misses, run `kache why-miss <crate>` from that worktree, using a Rust crate name from the report. It compares the latest recorded key material for that crate. When that worktree has no earlier build of the crate and both builds ran with `[cache] explain_miss` on, it compares with the first worktree's build instead. Check the reported input changes and the [cache limits](/docs/how-it-works/limits).
...
Add `--last-build` to report only the latest session. Open `reuse.md` before sharing it: reports include local paths and crate names, and store inventory covers the whole cache. The command writes a local file; publication is a separate action.
...
kache init --no-shell    # leave shell startup files unchanged
kache init --yes         # accept defaults without prompts
```

Init asks before editing Cargo or shell configuration and backs up existing files. If you enable C/C++ caching or test pacing, open a new terminal afterward. For C/C++ you can instead run the activation command it prints. `--check` shows the planned changes without prompts or modifications.
