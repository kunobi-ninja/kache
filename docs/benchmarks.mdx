...
| --- | --- | --- | --- |
| Same-repository PR, ready for review | hk, eza | Kache head and base: 2 cold, 6 same-checkout warm, 6 cross-checkout warm. sccache and mbx: 1 of each | 1 cold + 1 warm batch |
...
cron. Each takes five to six hours, and a workflow run counts as finished only
when its slowest job is, so keeping them beside the Linux matrix held every
other arm's telemetry behind them.
...
`mise.toml`, so the arm measures the same binary every night while only Kache
moves, and Kache against Kache is what the per-pull-request perf gate already
reports. Running it separately also keeps its 75 minutes off the nightly's
...
reported as an improvement. Other timing results are **inconclusive**. Every PR
runs head and base six times, so both warm phases get a timing verdict; the two
cold samples are reported without one. sccache and mbx run once, since they
never decide the verdict. The bench label runs every tool six times.
