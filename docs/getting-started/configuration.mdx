...
pass Kache's checks: some source must show `env!("VAR")` inside an `include!`,
`include_str!` or `include_bytes!` argument, and no source may read the value
anywhere else.
...
its artifacts. `CARGO_MANIFEST_DIR` is never accepted in either form.
