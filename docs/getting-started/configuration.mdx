...
Every GC driver (daemon sweep, auto-GC worker, `kache gc` without a daemon) records its run in `gc_stats.json` in the cache dir, including evictions that failed and how many of those lost the index write lock to running builds.
