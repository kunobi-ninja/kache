...
The payload also carries this host's shared cache: index size and a rowid high-water mark per table (`kache.cache.index.*`; it grows with every insert and replacement, so it is not a row count) and GC outcomes (`kache.cache.gc.*`: last run, plus cumulative totals including evictions that failed and those that lost the index write lock). Those figures never wait on a busy index.
