...
Timeline schema 2 records the session, plan, candidate source, and zero-based
...
Upgrade the service before sending schema 2 records. The updated service also
accepts schema 1. Current demand transfers and older logs still require a
unique key or time-window match; ambiguous transfers remain unattributed.
Packed-prefetch transfers and exact first-demand timing are not yet covered
by this attribution.
