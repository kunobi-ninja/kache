...
Kache remembers each unit's source and header inputs so an unchanged build can reuse its preprocessor result. Identical inputs share one database row, including the local file stamp and the raw and prefix-mapped content hashes. A hit checks the stamp first, then raw content, then mapped content.
