...
The pull-request comment opens with one table: for each subject, the base
branch and this PR as median times per build, and the change in words, such as
"17% slower, unconfirmed". A change is confirmed only when the paired test
below clears it. Timings for every tool, contention counters, disk use and the
paired test sit in folded sections below it; with more than one sample, each
time shows its range. The comment names this PR and the base branch; the data
calls them `head` and `base`.
