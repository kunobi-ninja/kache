...
| `KACHE_GC_MAX_AGE_HOURS` | `cache.gc_max_age_hours` | `168` (7 days) | Age-retention hours applied automatically by unattended GC. `0` disables automatic age eviction. An explicit `kache gc --max-age <duration>` continues to use the requested duration |
