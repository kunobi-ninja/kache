...
GC first flushes any entries still pending durability (see `kache flush-durability`).

## `kache flush-durability`

```bash
kache flush-durability
```

Flushes entries that a miss stored without an fsync (`cache.deferred_durability`), then exits once the queue has stayed empty for a few seconds. The wrapper starts one of these per store after a miss; it is hidden from `kache --help` because nothing else needs to run it. A second flusher for the same store exits at once because the first holds the lock. A hit on an entry the flusher has not reached yet verifies the entry's bytes before restoring.
