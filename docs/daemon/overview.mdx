...
With `cache.daemon_publish` enabled (the default), a C/C++ wrapper can hand its private output snapshots to the daemon. The compiler still runs in the wrapper. If the daemon cannot accept the work within 25 ms, the wrapper keeps the local publication path. Volume-local stores also use that path.
...
The daemon also binds a lifecycle control endpoint at `<runtime_dir>/daemon.ctl`. It is deliberately shorter than `daemon.sock`, so a runtime directory that fits one fits both.

Use a short, user-owned runtime directory. Unix socket paths are length-limited — 103 bytes on macOS, 107 on Linux — and `kache daemon start` refuses a longer one rather than leaving a daemon that cannot bind. Shared temporary directories also allow another local user to interfere with adjacent lock and state files.
