...
Timeline schema 4 records the session, plan, candidate source, and zero-based
...
including a restart that kills the old process before it finishes draining,
can prevent the final write.

Upgrade the service before sending schema 4 records. The updated service also
accepts schemas 1 through 3. Wrapper events before schema 20 have no demand
...
Packed-prefetch transfers are not yet covered by this attribution.
