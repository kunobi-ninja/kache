...
| `KACHE_PLANNER_GITHUB_AUDIENCE` | `cache.planner.github_audience` | planner base URL | Audience requested for the GitHub Actions ID token |
...
2. In GitHub Actions, the job's OIDC ID token. Its audience is the planner's base URL unless `KACHE_PLANNER_GITHUB_AUDIENCE` says otherwise. The job needs `permissions: id-token: write`, and the planner must trust the repository's owner.
