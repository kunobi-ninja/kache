...
Kache caches conservative, single-source `nvcc -c` object compilations. If it cannot prove that an invocation is safe to cache, it runs the real compiler unchanged.
...
- the modeled flags verbatim — compile mode, GPU `-arch` / `-gencode` / `-code`, `-std`, `-O`, defines, and the `-Xcompiler` / `-Xptxas` forwarding flags;
...
- relocatable and separable device code (`-rdc`, `-dc`);
- device debug (`-G`);
