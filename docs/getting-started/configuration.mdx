...
- On Unix, Kache records the binary's peak memory. A binary that used more than 512 MiB takes at least one slot per 512 MiB on its next run.
...
- Builds and test binaries started inside a test use that test's slots and wait for none of their own. A compile heavier than the reserve takes only the reserve while tests hold slots, so a test waiting on a build does not block it.
