...
| `KACHE_PLANNER_GITHUB_AUDIENCE` | `cache.planner.github_audience` | `kache` | Audience requested for the GitHub Actions ID token |
...
2. In GitHub Actions, the job's OIDC ID token for `KACHE_PLANNER_GITHUB_AUDIENCE` (default `kache`). The job needs `permissions: id-token: write`, and the planner must trust the repository's owner.
3. Your Kunobi session. Run `kache login` once; the daemon refreshes it quietly from then on. Use `kache login --device` on a machine without a browser, and `kache logout` to revoke it.
