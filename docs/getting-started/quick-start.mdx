...
`init` configures Cargo's Rust wrapper and, when unset, `HOST_CC` / `HOST_CXX` for build-script C and C++. On Unix it also offers compiler-name shims and [test pacing](/docs/getting-started/configuration#test-runner), and everywhere a login service. It does not set `CC` or `CXX`. See [C and C++](/docs/getting-started/c-cpp) for native build setup.
