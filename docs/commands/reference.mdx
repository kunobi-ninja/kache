...
Freed-space totals (`Selected:` in the selector, `would free` in a dry run, and `freed` after removal) count only bytes that return to disk: storage kache shares with the directory (hardlink or reflink) is kept by the store blob and reported as `shared with the store is kept`, not as freed.
